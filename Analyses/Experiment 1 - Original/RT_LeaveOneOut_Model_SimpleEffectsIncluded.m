...
% + WITHIN-CATEGORY SIMPLE EFFECTS FROM OMNIBUS MODEL
%% =========================================================
...
valid_predictors = ...
    ...
    ~isnan(rt_data.MeanS_z) & ...
    ~isnan(rt_data.MeanV) & ...
    ~isnan(rt_data.HueX_z) & ...
    ~isnan(rt_data.HueY_z);
...
M.MeanS_z    = double(rt_data.MeanS_z(:));
M.MeanV      = double(rt_data.MeanV(:));      % kept in table in case needed later
M.MeanL_z    = double(rt_data.MeanL_z(:));
M.HueX_z     = double(rt_data.HueX_z(:));
M.HueY_z     = double(rt_data.HueY_z(:));
...
formulas = { ...
    ['logRT ~ Category * ImageType + MeanL_z + AreaProp_z + MeanS_z + HueX_z + HueY_z + ' ...
     '(1|Participant) + (1|ObjectKey)']; ...
    ['logRT ~ Category * ImageType + MeanL_z + MeanS_z + HueX_z + HueY_z + ' ...
     '(1|Participant) + (1|ObjectKey)']; ...
    ['logRT ~ Category * ImageType + AreaProp_z + MeanS_z + HueX_z + HueY_z + ' ...
     '(1|Participant) + (1|ObjectKey)']; ...
    ['logRT ~ Category * ImageType + MeanL_z + HueX_z + HueY_z + AreaProp_z + ' ...
     '(1|Participant) + (1|ObjectKey)']; ...
    ['logRT ~ Category * ImageType + MeanL_z + MeanS_z + AreaProp_z + ' ...
     '(1|Participant) + (1|ObjectKey)']; ...
...
for i = 1:nModels
    ...
    idx_main = find(coef_names == "ImageType_Rest", 1);
    ...
    for c = 1:numel(cat_levels)
        ...
        % build contrast L
        L = zeros(1, numel(beta_hat));
        L(idx_main) = 1;

        % robust interaction search:
        % find any coefficient name that contains BOTH this category and ImageType_Rest
        % excluding the main ImageType_Rest coefficient
        idx_inter = find( ...
            contains(coef_names, "ImageType_Rest") & ...
            contains(coef_names, "Category_") & ...
            contains(coef_names, thisCat) );
        ...
        % exact hypothesis test for this linear contrast
        [p_val, F_val, df1_val, df2_val] = coefTest(lme_tmp, L);

        % signed t from F (df1 should be 1 here)
        t_val = sign(beta_val) * sqrt(F_val);

        % 95% CI (using df2 from coefTest)
        tcrit = tinv(0.975, df2_val);
...
% SAVE OUTPUTS
%% -------------------------------------------------
writetable(loo_summary, 'RT_leave_one_out_summary.csv');
writetable(all_loo_coefs, 'RT_leave_one_out_coefficients.csv');
writetable(all_loo_anova, 'RT_leave_one_out_ANOVA.csv');
writetable(simple_effects_summary, 'RT_simple_effects_within_category_by_LOO.csv');
...
% model_order = {'Full','NoArea','NoMeanL','NoMeanS','NoHue','Baseline'};
% cat_order   = {'Animal','Appliance','Clothing','Furniture', ...
%                'Musical Instrument','Tool','Vegetable','Vehicle'};
% 
